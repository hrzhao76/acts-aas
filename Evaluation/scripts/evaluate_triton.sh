...
mkdir -p $output_dir
cp -r $INSTALLDIR/model_repo/ $output_dir/
sed -i "s/count: 1/count: ${n_instance_per_gpu}/" $output_dir/model_repo/ActsExaTrkX/config.pbtxt

gpus_array=$(seq 0 $((n_gpus - 1)) | tr '\n' ',' | sed 's/,$//')
sed -i "/gpus:/c\    gpus: [ $gpus_array ]" $output_dir/model_repo/ActsExaTrkX/config.pbtxt
...
nohup tritonserver --model-repository=$output_dir/model_repo/ > ${log_filename} 2>&1 &
server_pid=$!
sleep 20
