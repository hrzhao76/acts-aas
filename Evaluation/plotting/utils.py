...
def plot_backend(
    backend_type: str,
    backend_results_path: Union[str, Path],
    ...
    different_ins_results_path = sorted(
        [item for item in backend_results_path.iterdir() if item.is_dir()]
    )
    ...
    for sync_mode in sync_modes:
        ...
        fig.savefig(
            output_path
            / f"perf_vs_concurrency_{backend_type}_{n_gpus_label}_{sync_mode}.png",
            dpi=300,
        )
...
def plot_backend_compare(
    custom_backend_results: Union[str, Path],
    ensemble_backend_results: Union[str, Path],
    var_name: str = "Inferences/Second",
    output_path: Union[str, Path] = None,
) -> None:
    ...
    different_ins_results_path = sorted(
        [item for item in custom_backend_results.iterdir() if item.is_dir()]
    )
    ...
    for sync_mode in sync_modes:
        ...
        for instance_result_path in different_ins_results_path:
            ...
            ensemble_instance_result_path = (
                ensemble_backend_results / f"ensemble_{instance_result_path.stem}"
            )
            ensemble_instance_result_path = check_inputpath(
                ensemble_instance_result_path
            )
            ...
            ensemble_csv_file = sorted(
                ensemble_instance_result_path.glob(csv_file_pattern)
            )
            ...
            # Plot
            ax_all.plot(
                custom_df_csv["Concurrency"],
                custom_df_csv[var_name],
                marker="o",
                label=f"{instance_label} custom backend",
            )
            ax_all.plot(
                ensemble_df_csv["Concurrency"],
                ensemble_df_csv[var_name],
                marker="^",
                label=f"{instance_label} ensemble scheduler",
            )
            ...
            ax.plot(
                ...
                ensemble_df_csv[var_name],
                marker="^",
                label="ensemble scheduler",
            )

            ax.set_xlabel("Concurrency")
            ax.set_ylabel(var_name)
            ax.set_title(
                f"{var_name} vs Concurrency, {instance_label}, {sync_mode} mode"
            )

            ax.legend()
            fig.savefig(
                output_path / f"perf_vs_concurrency_{instance_label}_{sync_mode}.png",
                dpi=300,
            )
        ...
        ax_all.legend()
        fig_all.savefig(
            output_path / f"perf_vs_concurrency_all_{sync_mode}.png", dpi=300
        )


def exatract_throughput_vs_instances(
    backend_results_path,
    pattern=r"(\d+)insts",
    sync_mode="sync",
    var_name="Inferences/Second",
    n_instance_threshold=10,
):
    backend_results = {
        "n_instances": [],
        "Throughput_mean": [],
...
def plot_timing_breakout(
    df: pd.DataFrame, timing_items: list = timing_items, fig=None, ax=None
):
    df = df.copy()
